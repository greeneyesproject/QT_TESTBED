...
int VisualFeatureDecoding::decodeKeyPoints(vector<uchar> bitstream,
                                           ...
                                           bool decodeAngles)
{
	...
	for(int n=0; n<nkpts; n++){
			...
			if( i<nbits_x ){
                ...
                if(decodeAngles==true){
                    if(i==nbits_x + nbits_y + nbits_s){
                        ...
                        qa |= (cur_bit << (nbits_x + nbits_y + nbits_s + nbits_a - i - 1) );
                    }
                }
            }
        ...
        if(decodeAngles==false){
            ...
            kpts.push_back( KeyPoint( (float)qx/4.0, (float)qy/4.0, (float)qs/4.0, (float)qa  ) );
        }
