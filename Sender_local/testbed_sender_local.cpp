...
int main(int argc, char ** argv) {
    ...
    int RADIO_DST = 0;
    cout << "apro..." << endl;
    serial_source src = dataTx->openSerialRADIO("/dev/ttyUSB1",115200,1);
    cout << "fatto!" << endl;
    ...
    while(1){
        ...
        if (curState == WAIT_CMD){
            ...
            while(!cmdOK){
                ...
                if( curCmd == START ){
                    ...
                    cmdOK = true;
                    featExtract->setDetThreshold(detectorName, opActs->det_threshold);
                    curState = BCKG_EXTR;
                    prevState = WAIT_CMD;
                }
                else if ( curCmd == STOP ){
            ...
            if( opActs->stop_program == true ){
                ...
                // if no objects are present
                    ...
                    if (prevState != IMG_ACQ){
                        ...
                        dataTx->sendNoObjPktRADIO(src,RADIO_DST);
                    }
        ...
        // -----------------------------------------------------------------------
            ...
            if(prevState == IMG_ACQ){
                ...
                featExtract->cutFeatures(kpts,features,opActs->max_features);
                cout << "OK" << endl;
                ...
                for(int i=0; i<B; i++){
                    ...
                    if ( opActs->do_compression == true ){
                        encoder.encodeKeyPoints(cur_kpts, cur_kp_bitstream,curW,curH,true);
                ...
                if(features.rows > 0){
                    // Send the image info packet
                    dataTx->sendImgInfoRADIO(src,RADIO_DST,imgID,true,opActs->do_compression,descriptorName,features.rows,curW,curH);
                    // Update the state
        ...
        // -----------------------------------------------------------------------
            ...
            cout << "Waiting the ACK......" << endl;
            ackOK = false;

            ackOK = dataTx->waitACK_RADIO(src,ACK_REC_INFO,imgID,ACK_DEADLINE);
            if (!ackOK){
                cout << "TIMEOUT EXPIRED... NOT requesting a new INFO ACK" << endl;
        ...
        // -----------------------------------------------------------------------
            ...
            for(unsigned int i=0; i< kp_bitstream.size(); i++){
                ctrl = dataTx->sendFeaturesRADIO(src,RADIO_DST,imgID,num_feats[i],kp_bitstream[i],ft_bitstream[i]);
        ...
        // -----------------------------------------------------------------------
            ...
            cout << "Send completed. Waiting the ack......" << endl;

            ackOK = false;

            ackOK = dataTx->waitACK_RADIO(src,ACK_REC_IMG,imgID,ACK_DEADLINE*2,opActs);
            if (!ackOK){
                cout << "TIMEOUT EXPIRED... NOT requesting a new ACK.. coming back to IMG_ACQ" << endl;

            }

            else{
                cout << "ack ok! setting new actions" << endl;
