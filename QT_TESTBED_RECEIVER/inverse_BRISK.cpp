...
inverse_BRISK::inverse_BRISK(VisualFeatureExtraction *ext, int imWidth, int imHeight){
...
int inverse_BRISK::build_database(string path){
	...
	for(unsigned int i=0; i<objects.size(); i++){
		...
		for(unsigned int j=0; j<db_files.size(); j++){
			...
			Mat tmp_desc;
            extractor->extractFeatures(imgGray,&tmp_kpts,&tmp_desc);
...
int inverse_BRISK::get_features(string inputImg, Mat &descriptors, vector<KeyPoint> &keypoints){
    ...
    // Extract keypoints and descriptors
	keypoints.clear();
    extractor->extractFeatures(imgGray,&keypoints,&descriptors);
